package com.tuling.juc.threadpool;

import com.tuling.juc.MonkeyRejectedExecutionHandler;
...
public class ThreadPoolDemo {
    public static void main(String[] args) {
        ...
        ThreadPoolExecutor threadPoolExecutor = new ThreadPoolExecutor(10, 20,
                0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>(10),new MonkeyRejectedExecutionHandler());//自定义线程池

        for (int i = 1; i <= 100; i++) {
            threadPoolExecutor.execute(new MyTask(i)); // new MyTask(i) 任务
        }
    }
}
